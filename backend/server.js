...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // Allow ALL Vercel domains (including arvindh-pizza-ten.vercel.app)
    ...
    if (process.env.FRONTEND_URL) {
      ...
      for (const allowedUrl of allowedUrls) {
        ...
        if (requestOrigin === allowedOrigin || requestOrigin.startsWith(allowedOrigin)) {
          console.log(`✓ CORS: Allowing FRONTEND_URL origin: ${origin}`);
          return callback(null, true);
        }
      }
    }
    ...
    if (process.env.NODE_ENV === 'production') {
      console.warn(`⚠️ CORS: Unknown origin: ${origin} - allowing anyway`);
